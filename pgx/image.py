...
# loads from file, returns image, optional convert argument
def load(filepath, convert=True):
    if filepath.startswith("http"):
        import requests  # only import if needed

        r = requests.get(filepath)
        filelike = io.BytesIO(r.content)
        ext = filepath.split(".")[-1]
        pic = pygame.image.load(filelike, ext)
    else:
        pic = pygame.image.load(path.handle(filepath))

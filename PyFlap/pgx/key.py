...
class key:
    ...
    def _prepare():
        ...
        for pg_event in event.get():
            if pg_event.type == pygame.KEYDOWN:
                pg_event.key = key.__comp_replace(pg_event.key)
                downevents.append(pg_event)
                key.pressed_memory.add(pg_event.key)

            if pg_event.type == pygame.KEYUP:
                pg_event.key = key.__comp_replace(pg_event.key)
                upevents.append(pg_event)
                key.pressed_memory.discard(pg_event.key)
